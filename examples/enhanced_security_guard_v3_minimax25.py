...
#   [FIX] Label positioning and text color contrast - labels now properly inside bounding boxes
#
# Version: 3.1.0
...
10. Fixed label positioning inside bounding boxes with high-contrast text colors
...
LABEL_TEXT_COLOR = (0, 0, 0)  # Black text for all labels
...
class FaceAttributeAnalyzer:
    ...
    def format(age: float, gender: float, emotion: Optional[np.ndarray] = None,
        ...
        if pose is not None and len(pose) >= 3:
            try:
                pose_str = f" P:{int(pose[0])} Y:{int(pose[1])}"
            except Exception:
                pass
...
# [V3.1 FIXES] label positioning inside bounding boxes, text color contrast
# ============================================================================
...
class EnhancedSecurityGuard(solutions.VisionEye):
    ...
    def _build_label(self, name: str, is_known: bool, attributes: Dict, confidence) -> str:
        """
        [V3.1 FIX] Build a single-line label (no newlines) for proper positioning.
        Uses spaces instead of newlines to ensure labels are positioned correctly
        ...
        if self.config.show_attributes and attributes:
            ...
            # [V3.1 FIX] Use space instead of newline for single-line label
            label = f"{label} {attr_str}"
        return label

    def _annotate_person(self, annotator, box, label, person_id, person_box, color):
        """
        Annotate person with label properly positioned inside bounding box.
        Uses black text color for readability.
        """
        ...
        annotator.box_label(box, label=final_label, color=color, txt_color=LABEL_TEXT_COLOR)
    ...
    def _annotate_object(self, annotator, cls, tid, box, conf):
        """Annotate non-person objects with black text."""
        label = f"cls:{int(cls)} id:{int(tid)} {float(conf):.2f}"
        annotator.box_label(box, label=label, color=(200, 200, 200), txt_color=LABEL_TEXT_COLOR)
