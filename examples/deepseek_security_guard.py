...
class NewPersonTracker:
    ...
    def update(self, person_ids, face_embeddings, face_boxes):
        ...
        for pid in current_ids:
            ...
            if self._matches_known_face(face_embeddings[pid]):
                # It's actually a returning person, so don't mark as new
                self._seen.add(pid)  # but still add to seen to avoid future checks
            else:
                new_ids.append(pid)
                self._new.add(pid)
        self._seen.update(current_ids)
        return new_ids
...
class FaceRecognizer:
    ...
    def __init__(
        ...
        tolerance: float = 0.5,
        embedding_smoothing_frames: int = 5,
        min_confidence: float = 0.65,  # new parameter
    ):
...
class EnhancedSecurityGuard(solutions.VisionEye):
    ...
    def _initialize_stats(self) -> None:
        ...
        self.stats = {
            ...
            "screenshots_saved": 0,
            "alerts_triggered": 0,
            "skipped_low_quality": 0,  # new stat
            "skipped_yaw": 0,  # new stat
        }
            ...
            if face_idx is not None and face_idx < len(face_embeddings):
                ...
                if self.screenshot_capturer:
                    ...
                    if quality < self.config.min_recognition_quality:
                        ...
                        # Optionally still cache as unknown? We'll return Unknown but not cache.
    ...
    def __call__(self, im0: np.ndarray) -> SolutionResults:
        ...
        if run_fr:
            ...
            # Face tracker update is currently unused; can be removed if not needed.
            # if self.config.enable_face_tracking and face_embeddings:
            #     self.face_tracker.update(face_boxes, face_embeddings, face_attributes)
        ...
        # Handle alarm with proper debouncing and state management
        alarm_threshold = self.CFG.get("records", 1)
        debounce_threshold = 3  # Number of consecutive frames before triggering alarm

        # Initialize alarm tracking state if not exists
        if not hasattr(self, "_alarm_pending"):
            self._alarm_pending = 0
        if not hasattr(self, "_alarm_triggered_this_frame"):
            self._alarm_triggered_this_frame = False

        # Determine alarm state based on unknown count
        alarm_condition_met = unknown_count >= alarm_threshold

        if alarm_condition_met:
            # Increment debounce counter for consecutive alarm-worthy frames
            self._alarm_pending += 1

            # Only trigger alarm after debounce threshold is reached
            # This prevents rapid re-triggering and false alarms
            ...
            # Reset debounce counter when condition is not met
            # This requires the threshold to be reached again before next alarm
            self._alarm_pending = 0
            self._clear_alarm()
