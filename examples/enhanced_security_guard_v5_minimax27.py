...
class FaceScreenshotCapturer:
    ...
    def __init__(
        ...
        # Create timestamped subdirectory for unique execution
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        timestamped_dir = os.path.join(output_dir, timestamp)
        self.known_dir = os.path.join(timestamped_dir, "known")
        self.unknown_dir = os.path.join(timestamped_dir, "unknown")
...
class FaceRecognizer:
    ...
    def __init__(
        ...
        # Voting buffer for consistent classification (majority vote over last 5 frames)
        self._vote_buffer: Dict[int, List[Tuple[str, bool, float]]] = {}
    ...
    def identify(self, embedding: np.ndarray, track_id: Optional[int] = None) -> Tuple[str, bool, float]:
        ...
        if track_id is not None:
            with self._identity_lock:
                ...
                current_name = best_name if "best_name" in locals() else "Unknown"
                current_is_known = "best_name" in locals() and best_score >= self.min_confidence
                current_conf = best_score if "best_score" in locals() else 0.0
                self._vote_buffer[track_id].append((current_name, current_is_known, current_conf))
                if len(self._vote_buffer[track_id]) > 5:
                    self._vote_buffer[track_id].pop(0)
                if len(self._vote_buffer[track_id]) >= 3:
                    known_votes = sum(1 for _, is_k, _ in self._vote_buffer[track_id] if is_k)
                    if known_votes >= len(self._vote_buffer[track_id]) // 2 + 1:
                        names = [n for n, is_k, _ in self._vote_buffer[track_id] if is_k and n != "Unknown"]
                        if names:
                            voted_name = max(set(names), key=names.count)
                            avg_conf = sum(c for _, _, c in self._vote_buffer[track_id]) / len(
                                self._vote_buffer[track_id]
                            )
                            return voted_name, True, avg_conf
